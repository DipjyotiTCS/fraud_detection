...
║  Transforms raw transaction records → 46-dimensional training matrix        ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  Pipeline stages:                                                            ║
...
class XGBoostFeatureEngineer:
    ...
    def transform(self, df: pd.DataFrame, is_training: bool = True) -> Tuple[pd.DataFrame, np.ndarray]:
        ...
        assert X.shape[1] == len(XGBOOST_ENGINEERED_FEATURES), f"Expected {len(XGBOOST_ENGINEERED_FEATURES)} features, got {X.shape[1]}"
...
class DatasetSplitter:
    ...
    def split(self, df: pd.DataFrame, X: pd.DataFrame, y: np.ndarray,
        ...
        if self.temporal_split:
            ...
            # Keep original row positions so df, X, y, and weights remain aligned.
            sorted_positions = df.sort_values("transaction_dt").index.to_numpy()
            ...
            idx_train = sorted_positions[:val_start]
            idx_val   = sorted_positions[val_start:train_val_end]
            idx_test  = sorted_positions[train_val_end:]
        else:
